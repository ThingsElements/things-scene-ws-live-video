...
export default class WSLiveVideo extends Rect {
  ...
  get playImage() {
    if(!this._playImage) {
      this._playImage = new Image();
      this._playImage.src = PLAY_BUTTON_IMAGE;
    }
  ...
  get stopImage() {
    if(!this._stopImage) {
      this._stopImage = new Image();
      this._stopImage.src = STOP_BUTTON_IMAGE;
    }
  ...
  drawPlayButton(ctx) {
    this.drawActionButton(ctx, this.playImage);
  }

  drawStopButton(ctx) {
    this.drawActionButton(ctx, this.stopImage);
  }

  drawActionButton(ctx, image) {

    var imageWidth = image.width
    var imageHeight = image.height

    // image 기본 크기보다 model의 크기가 작아질 경우
    if(image.width > this.model.width) {
      this.model.width = image.width
    }
    if(image.height > this.model.height) {
    ...
    ctx.drawImage(
      ...
      image.width,
      image.height,
      this.model.left + this.model.width / 2 - imageWidth /2 ,
      this.model.top + this.model.height /2 - imageHeight/2,
      imageWidth,
      imageHeight
      // image.width,
      // image.height
