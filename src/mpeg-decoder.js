...
export default class MpegDecoder {
  ...
  copyValueToDestination(value, destArray, destIndex, scan) {
    for( var n = 0; n < 64; n += 8, destIndex += scan+8 ) {
  ...
  addValueToDestination(value, destArray, destIndex, scan) {
    for( var n = 0; n < 64; n += 8, destIndex += scan+8 ) {
      destArray[destIndex+0] += value;
      destArray[destIndex+1] += value;
      destArray[destIndex+2] += value;
  ...
  copyBlockToDestinationClamp(blockData, destArray, destIndex, scan) {
    var n = 0;
    for( var i = 0; i < 8; i++ ) {
      for( var j = 0; j < 8; j++ ) {
        var p = blockData[n++];
  ...
  addBlockToDestinationClamp(blockData, destArray, destIndex, scan) {
    var n = 0;
    for( var i = 0; i < 8; i++ ) {
      for( var j = 0; j < 8; j++ ) {
  ...
  copyMacroblock(motionH, motionV, sY, sCr, sCb ) {
    var
      width, scan,
      H, V, oddH, oddV,
      src, dest, last;
  ...
  decodePicture(decodeCallback, skipOutput) {
    this.currentFrame++;
